...
class FirestoreService {
  ...
  Future<void> addArticle(Article article) async {
    final docId = firestore.collection(articlesCol).doc().id;
  ...
  Future<void> saveFavoriteArticle(Article article) async => await firestore
      ...
      .doc(article.id)
      .set(article.toMap(article.id!));

  Future<void> addCategoryToList(String category) async {
    final categoriesListDocCheck =
        await firestore.collection('categories').doc('categoriesList').get();
    ...
    final categoriesListDoc =
        await firestore.collection('categories').doc('categoriesList').get();

    List<String> categoriesListFromFirebase =
        List.from(categoriesListDoc.data()?['categories']);
    ...
    if (ListEquality<String>()
        .equals(categoriesListFromFirebase, categoriesList)) {
      await firestore
          .collection('categories')
          .doc('categoriesList')
  ...
  Future<void> addArticleToCategory(Article article, String articleId) async {
    checkIfArticleImageIsNull(article)
        ? await firestore
            ...
            .doc(articleId)
            .set(article.toMapNoImage(articleId))
        : await firestore
            .collection('categories')
            .doc(article.category)
            .collection('categoryArticles')
            .doc(articleId)
            .set(article.toMap(articleId));
  }
  ...
  Future<bool> checkIfArticleSaved(String docId) async {
    var collectionRef =
        firestore.collection('users').doc(uid).collection('saved_articles');
  ...
  Future<void> deleteArticle(String id) async =>
      await firestore.collection(articlesCol).doc(id).delete();

  Future<void> deleteArticleFromCategories(String id, String category) async =>
      await firestore
          .collection('categories')
          .doc(category)
          .collection('categoryArticles')
